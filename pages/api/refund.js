...
export default async function handler(req, res) {
  ...
  try{
    // call the refund RPC; accept optional admin_id & reason in body
    const admin_id = req.body.admin_id || null;
    const reason = req.body.reason || null;
    const rpc = await svc.rpc('sp_refund_transaction', { p_tx_id: id, p_admin_id: admin_id, p_reason: reason }).single();
    if (rpc.error) {
